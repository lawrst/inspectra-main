...
class ProductInfoService {
  ...
  static async saveProductInfo(data) {
    try {
      const { product_id, name, description, weight, movement, status: inputStatus } = data
  ...
  static async getAllProducts(page = 1, limit = 20, startDate = null, endDate = null) {
    try {
      ...
      const products = await ProductInfo.find(filter).sort({ timestamp: -1 }).skip(skip).limit(limit)
