...
const create = async (req, res) => {
  try {
    ...
    if (!name || !username || !email || !senha) {
      res
        .status(400)
        .send({ mensagem: "Preencha todos os campos para registro!" });
    }
...
const update = async (req, res) => {
  try {
    ...
    if (!name && !username && !email && !senha) {
      res
        .status(400)
        .send({ mensagem: "Submeta pelo menos um campo para atualizar!" });
    }
    ...
    await useServices.updateService(id, name, username, email, senha);
