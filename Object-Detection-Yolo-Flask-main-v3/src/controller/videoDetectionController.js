...
class VideoDetectionController {
  ...
  // Função auxiliar para obter timestamp em GMT-3
  static getBrazilTimestamp() {
    const now = new Date();
    // Ajusta para GMT-3
  ...
  static async saveVideoDetection(req, res) {
    try {
      ...
      // Usa o timestamp fornecido ou gera um novo em GMT-3
      const brazilTimestamp =
        timestamp || VideoDetectionController.getBrazilTimestamp();
